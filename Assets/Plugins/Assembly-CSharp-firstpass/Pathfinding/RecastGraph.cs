...
namespace Pathfinding
{
	...
	public class RecastGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder, IFunnelGraph
	{
		...
		public Bounds GetTileBounds(int x, int z)
		{
			...
			result.SetMinMax(new Vector3((float)(x * tileSizeX) * cellSize, 0f, (float)(z * tileSizeZ) * cellSize) + forcedBounds.min, new Vector3((float)((x + 1) * tileSizeX) * cellSize, forcedBounds.size.y, (float)((z + 1) * tileSizeZ) * cellSize) + forcedBounds.min);
			return result;
		}
		...
		public List<ExtraMesh> GetSceneMeshes(Bounds bounds)
		{
			if ((tagMask != null && tagMask.Count > 0) || (int)mask != 0)
			{
				...
				bool flag = false;
				{
					...
					{
						if (item2.GetComponent<Renderer>().isPartOfStaticBatch)
						{
							flag = true;
						}
						else if (item2.GetComponent<Renderer>().bounds.Intersects(bounds))
						{
							Mesh sharedMesh = item2.sharedMesh;
							ExtraMesh item = default(ExtraMesh);
							item.matrix = item2.GetComponent<Renderer>().localToWorldMatrix;
							item.original = item2;
							if (dictionary.ContainsKey(sharedMesh))
							{
								item.vertices = dictionary[sharedMesh];
							...
							item.bounds = item2.GetComponent<Renderer>().bounds;
							list2.Add(item);
						}
						if (flag)
						{
		...
		public void UpdateAreaInit(GraphUpdateObject o)
		{
			...
			if (!CollectMeshes(out extraMeshes, bounds))
			{
			}
		...
		public void UpdateArea(GraphUpdateObject guo)
		{
			...
			IntRect a = new IntRect(Mathf.FloorToInt(bounds.min.x / ((float)tileSizeX * cellSize)), Mathf.FloorToInt(bounds.min.z / ((float)tileSizeZ * cellSize)), Mathf.FloorToInt(bounds.max.x / ((float)tileSizeX * cellSize)), Mathf.FloorToInt(bounds.max.z / ((float)tileSizeZ * cellSize)));
			...
			uint num3 = (uint)AstarPath.active.astarData.GetGraphIndex(this);
			for (int num4 = a.xmin; num4 <= a.xmax; num4++)
			{
				for (int num5 = a.ymin; num5 <= a.ymax; num5++)
				{
					NavmeshTile navmeshTile3 = tiles[num4 + num5 * tileXCount];
					GraphNode[] array = navmeshTile3.nodes;
					for (int num6 = 0; num6 < array.Length; num6++)
					{
						array[num6].GraphIndex = num3;
					}
				}
			}
			a = a.Expand(1);
			...
			for (int num7 = a.xmin; num7 <= a.xmax; num7++)
			{
				...
				{
					if (num7 < tileXCount - 1 && a.Contains(num7 + 1, num8))
					{
						ConnectTiles(tiles[num7 + num8 * tileXCount], tiles[num7 + 1 + num8 * tileXCount]);
					}
					if (num8 < tileZCount - 1 && a.Contains(num7, num8 + 1))
					{
						ConnectTiles(tiles[num7 + num8 * tileXCount], tiles[num7 + (num8 + 1) * tileXCount]);
					}
				}
			}
		}
