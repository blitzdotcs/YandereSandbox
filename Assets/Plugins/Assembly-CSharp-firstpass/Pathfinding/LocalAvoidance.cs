		...
		public class VO
		{
			...
			public bool FinalInts(Vector3 target, Vector3 closeEdgeConstraint, bool drawGizmos, out Vector3 closest)
			{
				...
				for (int i = 1; i <= 3; i++)
				{
					...
					float num3 = AstarMath.NearestPointFactor(vector7, vector9, target);
					...
					for (int j = 0; j < list2.Count - ((i == 3) ? 1 : 0); j++)
					{
						if (drawGizmos)
						{
							Debug.DrawRay(vector7 + (vector9 - vector7) * list2[j].factor, Vector3.down, (list2[j].state != IntersectionState.Outside) ? Color.red : Color.green);
						}
						if (list2[j].state == IntersectionState.Outside && ((j == list2.Count - 1 && (j == 0 || list2[j - 1].state != IntersectionState.Outside)) || (j < list2.Count - 1 && list2[j + 1].state == IntersectionState.Outside)))
						{
							flag2 = true;
							float factor = list2[j].factor;
							float num6 = ((j != list2.Count - 1) ? list2[j + 1].factor : ((i != 3) ? float.PositiveInfinity : 1f));
							if (drawGizmos)
							{
								Debug.DrawLine(vector7 + (vector9 - vector7) * factor + Vector3.up, vector7 + (vector9 - vector7) * Mathf.Clamp01(num6) + Vector3.up, Color.green);
							}
					...
					if (flag2)
					{
						...
						Vector3 vector10 = vector7 + (vector9 - vector7) * num7;
						if (!flag || (vector10 - target).sqrMagnitude < (closest - target).sqrMagnitude)
						{
							closest = vector10;
						}
						if (drawGizmos)
		...
		public Vector3 ClampMovement(Vector3 direction)
		{
			...
			if (resType == ResolutionType.Geometric)
			{
				...
				for (int l = 0; l < vos.Count; l++)
				{
					...
					list.Add(new VOLine(vO2, vector5, end, true, 1, num6 == 1));
					list.Add(new VOLine(vO2, vector6, end2, true, 2, num6 == 2));
					list.Add(new VOLine(vO2, vector5, vector6, false, 3, false));
					bool flag = false;
					bool flag2 = false;
					if (!flag)
					{
				...
				for (int num7 = 0; num7 < list.Count; num7++)
				{
					for (int num8 = num7 + 1; num8 < list.Count; num8++)
					{
						...
						float factor;
						float factor2;
						if (vOLine.vo == vOLine2.vo || !Polygon.IntersectionFactor(vOLine.start, vOLine.end, vOLine2.start, vOLine2.end, out factor, out factor2) || factor < 0f || factor2 < 0f || (!vOLine.inf && factor > 1f) || (!vOLine2.inf && factor2 > 1f))
						{
							continue;
						}
				...
				{
					Vector3 closest;
					if (!vos[num10].FinalInts(vector2, base.transform.position + velocity, drawGizmos, out closest))
					{
						continue;
					}
