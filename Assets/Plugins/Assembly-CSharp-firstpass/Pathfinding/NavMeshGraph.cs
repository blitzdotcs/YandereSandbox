	...
	public class NavMeshGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder, IFunnelGraph
	{
		...
		public static bool Linecast(INavmesh graph, Vector3 tmp_origin, Vector3 tmp_end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace)
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < triangleMeshNode.connections.Length; i++)
				{
					...
					if (triangleMeshNode.GetPortal(triangleMeshNode.connections[i], list, list2, false))
					{
						...
						float factor;
						float factor2;
						if ((Polygon.LeftNotColinear(vector, vector2, hit.origin) || !Polygon.LeftNotColinear(vector, vector2, tmp_end)) && Polygon.IntersectionFactor(vector, vector2, hit.origin, tmp_end, out factor, out factor2) && !(factor2 < 0f) && factor >= 0f && factor <= 1f)
						{
			...
			for (int j = 0; j < vertexCount; j++)
			{
				...
				float factor3;
				float factor4;
				if ((Polygon.LeftNotColinear(vector3, vector4, hit.origin) || !Polygon.LeftNotColinear(vector3, vector4, tmp_end)) && Polygon.IntersectionFactor(vector3, vector4, hit.origin, tmp_end, out factor3, out factor4) && !(factor4 < 0f) && factor3 >= 0f && factor3 <= 1f)
				{
