	...
	public class Voxelize
	{
		...
		public int Triangulate(int n, int[] verts, ref int[] indices, ref int[] tris)
		{
			...
			while (n > 3)
			{
				...
				{
					int num7 = Next(k, n);
					if (((uint)indices[num7] & 0x40000000u) != 0)
					{
		...
		public void VoxelizeInput()
		{
			...
			Matrix4x4 matrix4x = Matrix4x4.Scale(new Vector3(num, num2, num)) * Matrix4x4.TRS(-vector, Quaternion.identity, Vector3.one);
						...
						for (int num12 = num10; num12 <= num11; num12++)
						{
							...
							if (num13 >= 3)
							{
								...
								if (num17 >= 0)
								{
									...
									voxelArea.AddLinkedSpan(num12 * voxelArea.width + m, (num18 >= 0) ? ((uint)num18) : 0u, (uint)num17, area2, voxelWalkableClimb);
								}
							}
						}
					}
