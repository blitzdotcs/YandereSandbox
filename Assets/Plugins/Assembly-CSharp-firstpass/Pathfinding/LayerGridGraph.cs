...
namespace Pathfinding
{
	public class LayerGridGraph : GridGraph, IUpdatableGraph, IRaycastableGraph
	{
		...
		public new bool SnappedLinecast(Vector3 _a, Vector3 _b, GraphNode hint, out GraphHitInfo hit)
		{
			...
			LevelGridNode levelGridNode = GetNearest(_a, NNConstraint.None).node as LevelGridNode;
			LevelGridNode levelGridNode2 = GetNearest(_b, NNConstraint.None).node as LevelGridNode;
			...
			while (true)
			{
				...
				num = Math.Abs(@int.x - int2.x);
				num2 = Math.Abs(@int.z - int2.z);
				int num3 = 0;
				if (num >= num2)
				{
					num3 = ((int2.x > @int.x) ? 1 : 3);
				}
				else if (num2 > num)
				{
					num3 = ((int2.z > @int.z) ? 2 : 0);
				}
				...
				@int = (Int3)inverseMatrix.MultiplyPoint3x4((Vector3)levelGridNode4.position);
