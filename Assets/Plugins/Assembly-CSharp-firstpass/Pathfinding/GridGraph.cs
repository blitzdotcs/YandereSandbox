		...
		public override NNInfo GetNearestForce(Vector3 position, NNConstraint constraint)
		{
			...
			while (true)
			{
				...
				for (num10 = num3 - num9; num10 <= num3 + num9; num10++)
				{
					...
					{
						float sqrMagnitude = ((Vector3)nodes[num10 + num12].position - vector).sqrMagnitude;
						if (sqrMagnitude < num5 && sqrMagnitude < num8)
						{
							num5 = sqrMagnitude;
							gridNode2 = nodes[num10 + num12];
							clampedPosition = matrix.MultiplyPoint3x4(new Vector3(Mathf.Clamp(num, (float)num10 - 0.5f, (float)num10 + 0.5f) + 0.5f, inverseMatrix.MultiplyPoint3x4((Vector3)gridNode2.position).y, Mathf.Clamp(num2, (float)num11 - 0.5f, (float)num11 + 0.5f) + 0.5f));
						}
					}
				}
				num11 = num4 - num9;
				...
				for (num10 = num3 - num9; num10 <= num3 + num9; num10++)
				{
					...
					if (constraint.Suitable(nodes[num10 + num12]))
					{
						float sqrMagnitude2 = ((Vector3)nodes[num10 + num12].position - vector).sqrMagnitude;
				...
				for (num11 = num4 - num9 + 1; num11 <= num4 + num9 - 1; num11++)
				{
					...
					{
						float sqrMagnitude3 = ((Vector3)nodes[num10 + num11 * width].position - vector).sqrMagnitude;
				...
				for (num11 = num4 - num9 + 1; num11 <= num4 + num9 - 1; num11++)
				{
					...
					if (constraint.Suitable(nodes[num10 + num11 * width]))
					{
						float sqrMagnitude4 = ((Vector3)nodes[num10 + num11 * width].position - vector).sqrMagnitude;
