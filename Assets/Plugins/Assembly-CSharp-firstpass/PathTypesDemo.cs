...
public class PathTypesDemo : MonoBehaviour
{
	...
	public void OnGUI()
	{
		...
		if (activeDemo == 2 || activeDemo == 3)
		{
			...
			GUILayout.Label(((activeDemo != 2) ? "Flee strength" : "Aim strength") + " (" + aimStrength + ")");
	...
	public void OnPathComplete(Path p)
	{
		...
		else if (p.GetType() == typeof(ConstantPath))
		{
			...
			bool flag = false;
			List<Vector3> pointsOnNodes = PathUtilities.GetPointsOnNodes(allNodes, 20, 0f);
			Vector3 zero = Vector3.zero;
			for (int l = 0; l < pointsOnNodes.Count; l++)
			{
				Debug.DrawRay(pointsOnNodes[l], Vector3.up * 5f, Color.red, 3f);
				zero += pointsOnNodes[l];
			}
			if (pointsOnNodes.Count > 0)
			{
				zero /= (float)pointsOnNodes.Count;
			}
			for (int m = 0; m < pointsOnNodes.Count; m++)
			{
				List<Vector3> list4;
				List<Vector3> list5 = (list4 = pointsOnNodes);
				int index;
				int index2 = (index = m);
				Vector3 vector = list4[index];
				list5[index2] = vector - zero;
			}
			PathUtilities.GetPointsAroundPoint(start.position, AstarPath.active.astarData.graphs[0] as IRaycastableGraph, pointsOnNodes, 0f, 1f);
			for (int n = 0; n < pointsOnNodes.Count; n++)
			{
				Debug.DrawRay(pointsOnNodes[n], Vector3.up * 5f, Color.blue, 3f);
			}
			for (int num = allNodes.Count - 1; num >= 0; num--)
			{
				Vector3 vector2 = (Vector3)allNodes[num].position + pathOffset;
				...
				if (flag)
				{
					Debug.DrawRay(vector2, Vector3.up, Color.blue);
				}
				else
				{
					...
					list3.Add(vector2 + new Vector3(-0.5f, 0f, -0.5f) * num2);
					list3.Add(vector2 + new Vector3(0.5f, 0f, -0.5f) * num2);
					list3.Add(vector2 + new Vector3(-0.5f, 0f, 0.5f) * num2);
					list3.Add(vector2 + new Vector3(0.5f, 0f, 0.5f) * num2);
				}
			...
			int num3 = 0;
			int num4 = 0;
			for (; num3 < array.Length; num3 += 4)
			{
				array2[num4] = num3;
				array2[num4 + 1] = num3 + 1;
				array2[num4 + 2] = num3 + 2;
				array2[num4 + 3] = num3 + 1;
				array2[num4 + 4] = num3 + 3;
				array2[num4 + 5] = num3 + 2;
				num4 += 6;
			}
			Vector2[] array3 = new Vector2[array.Length];
			for (int num5 = 0; num5 < array3.Length; num5 += 4)
			{
				array3[num5] = new Vector2(0f, 0f);
				array3[num5 + 1] = new Vector2(1f, 0f);
				array3[num5 + 2] = new Vector2(0f, 1f);
				array3[num5 + 3] = new Vector2(1f, 1f);
			}
			mesh.vertices = array;
		...
		else
		{
			...
			for (int num6 = 0; num6 < p.vectorPath.Count; num6++)
			{
				component4.SetPosition(num6, p.vectorPath[num6] + pathOffset);
			}
